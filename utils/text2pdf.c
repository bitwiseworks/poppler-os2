...
REVISION HISTORY
Version 1.1.1 (by Herwig Bauernfeind)
06 Aug 2004 Added option to produce A5 format PDF files.

Version 1.1
11 Oct 1996 Added handling of form-feed characters, removed need for tmp file,
            put reference to resources in each page (avoid bug in Acrobat),
	        changed date format to PDF-1.1 standard.
12 Jun 1996 Added check to avoid blank last page
12 Jun 1996 Added LINE_END def to get round platform-specific \r, \n etc.
18 Mar 1996 Added ISOLatin1Encoding option
**************************************************************************/
...
void ShowHelp(){
  ...
  printf("  -A5\t\tuse A5 paper (default Letter)\n"); // Added by Herwig Bauernfeind
  ...
  printf("\n%s (c) Phil Smith, 1996, additions (c) Herwig Bauernfeind, 2004\n", appname);
}
...
int main(int argc, char **argv){
  ...
  while (i < argc) {
	...
	case '4':
	  ...
	  pageHeight = 842;
	  break;
	case '5': // added by Herwig Bauernfeind
	  pageWidth = 421;
	  pageHeight = 595;
	  break;
	default:
